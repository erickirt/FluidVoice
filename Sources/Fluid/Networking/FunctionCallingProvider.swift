...
final class FunctionCallingProvider {
    ...
    func processWithTools(
        ...
        if !tools.isEmpty {
            DebugLogger.shared.info("Tools in request: \(tools.map { ($0["function"] as? [String: Any])?["name"] as? String ?? "?" }.joined(separator: ", "))", source: "FunctionCallingProvider")
        }
        ...
        do {
            ...
            if let toolCalls = message.tool_calls, !toolCalls.isEmpty {
                DebugLogger.shared.info("🎯 LLM decided to call \(toolCalls.count) tools", source: "FunctionCallingProvider")
                var parsedCalls: [(name: String, arguments: [String: Any], callId: String)] = []
                
                for toolCall in toolCalls {
                    DebugLogger.shared.info("  → \(toolCall.function.name)(\(toolCall.function.arguments))", source: "FunctionCallingProvider")
            ...
            DebugLogger.shared.info("💭 LLM responded with text only (finish_reason: \(choice.finish_reason ?? "none"))", source: "FunctionCallingProvider")
            return .textResponse(message.content ?? "<no content>")
            
        } catch {
            DebugLogger.shared.error("❌ Function calling failed: \(error.localizedDescription)", source: "FunctionCallingProvider")
    ...
    func continueWithToolResults(
        ...
        do {
            ...
            if let http = response as? HTTPURLResponse, http.statusCode >= 400 {
                let errText = String(data: data, encoding: .utf8) ?? "Unknown error"
                DebugLogger.shared.error("HTTP \(http.statusCode) in continueWithToolResults: \(errText)", source: "FunctionCallingProvider")
                return .error("HTTP \(http.statusCode): \(errText)")
            }
            ...
            if let toolCalls = choice.message.tool_calls, !toolCalls.isEmpty {
                DebugLogger.shared.warning("Model tried to call tools in final response (tool_choice was 'none'). Ignoring tool calls.", source: "FunctionCallingProvider")
            ...
            DebugLogger.shared.error("Error in continueWithToolResults: \(error.localizedDescription)", source: "FunctionCallingProvider")
            return .error(error.localizedDescription)
        }
    }
}
